...
public class ThoiKhoaBieu {
    ...
    public ThoiKhoaBieu(int index) {
        ...
        if (currentWeek < 1 || currentWeek > 23) {
            throw new IllegalArgumentException("Tuần hiện tại không hợp lệ: " + currentWeek);
        }
        this.danhSachTuan = new ArrayList<>();
        this.semesterStartDate = calculateSemesterStartDate(index);
        System.out.println("Ngày bắt đầu học kỳ: " + semesterStartDate);
        System.out.println("Ngày bắt đầu tuần hiện tại (tuần " + currentWeek + "): " + calculateCurrentWeekStartDate(index));
    }

    private LocalDate calculateSemesterStartDate(int index) {
        LocalDate today = LocalDate.now();
        int dayOfWeek = today.getDayOfWeek().getValue();
        LocalDate mondayOfCurrentWeek = today.minusDays(dayOfWeek - 1);
        return mondayOfCurrentWeek.minusDays(index * 7);
    }
    ...
    public void themLichHoc(LichHoc lichHoc) {
        ...
        // Đệm chuỗi nếu ngắn hơn 23 ký tự
        if (thoiGianHoc.length() < 23) {
        ...
        for (int i = 0; i < thoiGianHoc.length() && i < 20; i++) {
            ...
            // Chấp nhận '1', '0' (tuần 20), '7', '8', '9' (tuần 17, 18, 19)
